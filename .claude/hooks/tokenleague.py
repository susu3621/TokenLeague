...
SESSION_FILE_NAME = ".tokenleague_session.json"
...
def _send_api_request(endpoint: str, payload: dict[str, Any]) -> bool:
    ...
    if not hook_key:
        # Silently fail if no hook key configured
        return False
...
def _extract_model_info(event_data: dict[str, Any]) -> tuple[str, str]:
    ...
    agent_version = event_data.get("claude_code_version") or event_data.get("agent_version") or "unknown"
